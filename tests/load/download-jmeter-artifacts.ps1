param(
  ...
  [string]$Workflow = "JMeter Tests",
  [string]$OutputDir = "data/jmeter-actions",
  [int]$Limit = 20
)
...
if ($LASTEXITCODE -ne 0) {
  throw "Gagal mengambil daftar run JMeter. Jalankan 'gh auth login' lalu coba lagi."
}
...
$targetDir = Join-Path $OutputDir ("jmeter-run-{0}-{1}" -f $runId, $timestamp)

New-Item -ItemType Directory -Path $targetDir -Force | Out-Null

& $gh run download $runId --repo $Repo --dir $targetDir
if ($LASTEXITCODE -ne 0) {
  throw "Gagal download artifact JMeter run $runId. Jalankan 'gh auth login' lalu coba lagi."
}

$renameMap = @{
  "result.jtl"          = "jmeter-result-run-$runId.jtl"
  "jmeter-summary.md"  = "jmeter-summary-run-$runId.md"
}
...
$manifestPath = Join-Path $targetDir ("jmeter-artifact-manifest-run-{0}.md" -f $runId)
@(
