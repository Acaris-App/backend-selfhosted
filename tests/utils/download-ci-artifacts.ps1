...
$targetDir = Join-Path $OutputDir ("backend-ci-run-{0}-{1}" -f $runId, $timestamp)
...
$renameMap = @{
  ...
  "health-smoke-summary.md" = "health-smoke-summary-run-$runId.md"
  "health-response.json"    = "health-response-run-$runId.json"
}
...
$manifestPath = Join-Path $targetDir ("ci-artifact-manifest-run-{0}.md" -f $runId)
@(
